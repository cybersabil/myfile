...
(() => {
  ...
  async function passwordPhoneApproval(login, device) {
    return await new Promise((resolve,reject)=>{
      ...
      card.className='cy-phone-approval-card cy-number-match-card';
      ...
      description.textContent=
        'CyberSabil notification me computer par dikh raha number select karein. Daily login me QR scan nahi hoga.';

      const number=document.createElement('div');
      number.className='cy-number-match-value';
      number.textContent='--';
      ...
      stateText.textContent='Secure number-match request prepare ho rahi hai…';
      state.append(spinner,stateText);

      const resend=document.createElement('button');
      resend.type='button';
      resend.className='cy-phone-button secondary';
      resend.textContent='Resend notification';
      ...
      card.append(icon,title,description,number,state,resend,totp,cancel);
      overlay.append(card);
      ...
      const setState=(text,tone='')=>{
        state.classList.toggle('cy-phone-error',tone==='error');
        state.classList.toggle('cy-phone-success',tone==='success');
      ...
      async function poll(){
        ...
        try{
          const r=await phonePasskeyFetch('/auth/password/push/status',{
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({mfaToken:login.mfaToken,challengeId})
          });

          if(r.status==='approved'&&r.sessionToken){
            stopTimers();
            setState('Approved ✓ Secure sign-in complete.','success');
            setTimeout(()=>{
              overlay.remove();
              resolve(r);
            },180);
          ...
          if(r.status==='denied'){
            stopTimers();
            setState('Wrong number selected. Request denied. Resend ya 6-digit fallback use karein.','error');
          ...
          if(r.status==='expired'){
            stopTimers();
            setState('Request expire ho gaya. Resend notification karein.','error');
            resend.disabled=false;
            enableFallback();
            return;
          }
          ...
          if(code==='PASSWORD_STEPUP_INVALID'){
            stopTimers();
            setState('Login request expire ho gaya. Cancel karke password se dobara sign in karein.','error');
            return;
          }
      ...
      async function startPush(){
        stopped=false;
        clearTimeout(pollTimer); clearTimeout(expiryTimer); clearTimeout(resendTimer);
        challengeId='';
        number.textContent='--';
        resend.disabled=true;
        totp.disabled=false;
        cancel.disabled=false;
        setState('Phone notification bheji ja rahi hai…');

        try{
          const r=await phonePasskeyFetch('/auth/password/push/start',{
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({mfaToken:login.mfaToken})
          });

          challengeId=r.challengeId;
          number.textContent=String(r.number).padStart(2,'0');
          setState(`Notification sent to ${r.deliveredDevices||1} phone. Matching number tap karein.`);
          resendTimer=setTimeout(()=>{ if(!stopped)resend.disabled=false; },10000);
          expiryTimer=setTimeout(()=>{
            if(stopped)return;
            setState('Request expire ho gaya. Resend notification karein.','error');
            resend.disabled=false;
          },Math.max(1000,(Number(r.expiresAt||0)*1000-Date.now())));
          poll();
        }catch(e){
          const code=String(e?.code||e?.message||'');
          if(code==='PUSH_NOT_ENROLLED'){
            setState('Phone Alerts abhi enrolled nahi hain. 6-digit fallback se login karke files me “Phone Alerts” se one-time setup karein.','error');
          }else if(code==='PUSH_DELIVERY_FAILED'){
            setState('Phone notification deliver nahi hui. Phone notification settings check karein ya 6-digit fallback use karein.','error');
          }else{
            setState('Phone approval start nahi hua. 6-digit fallback available hai.','error');
          }
          resend.disabled=false;
          enableFallback();
        }
      ...
      resend.onclick=()=>{ stopTimers(); startPush(); };
      totp.onclick=()=>{
        stopTimers();
        overlay.remove();
        resolve({mode:'totp'});
  ...
  async function installLoginUI() {
    ...
    // CYBERSABIL_LOGIN_REENABLE_AFTER_LOGOUT_V1_2
    // A successful password login deliberately disables the form to prevent
    // duplicate submissions. The same DOM panel is later reused after logout,
    // so reset it ONLY on the authLayer hidden -> visible transition.
    let authLayerWasHidden =
      authLayer.classList.contains('hidden');

    function resetLoginPanelAfterLogout() {
      clearInputError(user,pass);
      ...
      // Never retain the password across a completed authenticated session.
      pass.value='';

      // The old Turnstile token was consumed by the previous password login.
      loginTurnstileToken='';
      login.disabled=true;
      login.style.opacity='.68';
      login.style.cursor='not-allowed';
    ...
    const authRevealObserver =
      new MutationObserver(() => {
        ...
        if (
          ...
          resetLoginPanelAfterLogout();
        }

        authLayerWasHidden = nowHidden;
      });

    authRevealObserver.observe(
      authLayer,
      {
        attributes:true,
